...
from ..analysis_config import get_pose_backend
...
try:
    ...
    sys.modules[spec.name] = pose_module
...
class TurnSegment:
    ...
    end_time: float
    confidence: float
    mean_angular_speed_deg_s: float
...
class TurnAnalysisResult:
    """Result of turn-specific analysis for a segment."""
    ...
    motion_suppression_score: float
    rotation_motion_mismatch_score: float
    step_irregularity_score: float
    shuffle_step_score: float
    stall_during_rotation_score: float
...
class WalkingDetector:
    ...
    def __init__(self, min_duration: float = 1.0, confidence_threshold: float = 0.30):
        self.min_duration = min_duration
        self.confidence_threshold = confidence_threshold

    def detect(
        self, keypoints: np.ndarray, fps: float
    ) -> Tuple[List[WalkingSegment], np.ndarray, np.ndarray]:
        """Detect walking segments in pose sequence."""
        T = len(keypoints)
        if T < int(fps * 1.0):
            return [], np.zeros(T, dtype=bool), np.zeros(T, dtype=float)
        ...
        # Compute scores
        movement_score = self._compute_movement_score(hip_center, ankle_left, ankle_right, fps)
        ...
        # Combine
        walking_confidence = 0.2 * movement_score + 0.4 * rhythm_score + 0.4 * leg_motion_score
        ...
        return segments, is_walking, walking_confidence

    def _compute_movement_score(self, hip_center, ankle_left, ankle_right, fps):
        """Compute walking-related motion score in pelvis-aligned coordinates."""
        del hip_center

        # After translation alignment, pelvis displacement is near-zero by design.
        # Use foot excursion relative to the aligned body instead.
        ankle_center = (ankle_left + ankle_right) / 2.0
        ankle_separation = np.linalg.norm(ankle_left - ankle_right, axis=1)
        excursion = np.sqrt(np.sum(np.diff(ankle_center, axis=0) ** 2, axis=1))
        excursion = np.concatenate([[0], excursion])
        separation_change = np.abs(np.diff(ankle_separation))
        separation_change = np.concatenate([[0], separation_change])

        combined = excursion + 0.5 * separation_change
        smoothed = uniform_filter1d(combined, max(3, int(fps * 0.3)))
        positive = smoothed[smoothed > 0]
        if positive.size == 0:
            return np.zeros_like(smoothed)
        normalized = np.clip(smoothed / np.percentile(positive, 75), 0, 1)
        return normalized
...
class TurnDetector:
    """Detect turning segments from raw pose orientation changes."""
    ...
    def __init__(
        ...
        angular_speed_threshold: float = 45.0,
        min_rotation_span_deg: float = 55.0,
        expansion_speed_ratio: float = 0.45,
        padding_duration: float = 0.15,
        context_window_sec: float = 3.0,
        slow_rotation_span_deg: float = 12.0,
        slow_core_rotation_deg: float = 20.0,
        slow_direction_consistency: float = 0.60,
        slow_motion_percentile: float = 35.0,
        slow_motion_ratio: float = 0.45,
    ):
        self.min_duration = min_duration
        self.angular_speed_threshold = angular_speed_threshold
        self.min_rotation_span_deg = min_rotation_span_deg
        self.expansion_speed_ratio = expansion_speed_ratio
        self.padding_duration = padding_duration
        self.context_window_sec = context_window_sec
        self.slow_rotation_span_deg = slow_rotation_span_deg
        self.slow_core_rotation_deg = slow_core_rotation_deg
        self.slow_direction_consistency = slow_direction_consistency
        self.slow_motion_percentile = slow_motion_percentile
        self.slow_motion_ratio = slow_motion_ratio
    ...
    def detect(
        ...
        fps: float,
    ) -> tuple[list[TurnSegment], np.ndarray, np.ndarray]:
        n_frames = len(keypoints)
        if n_frames < max(5, int(fps)):
            return [], np.zeros(n_frames, dtype=bool), np.zeros(n_frames, dtype=float)

        body_angle = self._compute_body_angle(keypoints)
        angular_velocity = np.gradient(body_angle) * fps * (180.0 / np.pi)
        angular_speed = uniform_filter1d(np.abs(angular_velocity), max(3, int(fps * 0.25)))
        ankle_center = (
            keypoints[:, self.LEFT_ANKLE, :2] + keypoints[:, self.RIGHT_ANKLE, :2]
        ) / 2.0
        ankle_motion = np.sqrt(np.sum(np.diff(ankle_center, axis=0) ** 2, axis=1))
        ankle_motion = np.concatenate([[0.0], ankle_motion]) * fps
        ankle_motion = uniform_filter1d(ankle_motion, max(3, int(round(fps * 0.4))))

        quality_mask = pose_quality > 0.2
        core_mask = (angular_speed >= self.angular_speed_threshold) & quality_mask
        support_mask = (
            angular_speed >= self.angular_speed_threshold * self.expansion_speed_ratio
        ) & quality_mask
        context_rotation_deg, direction_consistency = self._compute_directional_context(
            body_angle, angular_velocity, fps
        )
        positive_motion = ankle_motion[(ankle_motion > 0) & quality_mask]
        slow_motion_threshold = (
            float(np.percentile(positive_motion, self.slow_motion_percentile))
            if positive_motion.size
            else 0.0
        )
        slow_motion_mask = (
            ankle_motion >= slow_motion_threshold * self.slow_motion_ratio
            if slow_motion_threshold > 0
            else np.ones_like(ankle_motion, dtype=bool)
        )
        slow_support_mask = (
            quality_mask
            & slow_motion_mask
            & (context_rotation_deg >= self.slow_rotation_span_deg)
            & (direction_consistency >= self.slow_direction_consistency)
        )
        slow_core_mask = (
            quality_mask
            & slow_motion_mask
            & (context_rotation_deg >= self.slow_core_rotation_deg)
            & (direction_consistency >= self.slow_direction_consistency)
        )
        core_mask = core_mask | slow_core_mask
        support_mask = support_mask | slow_support_mask

        # Build turn candidates from confident core motion, then expand slightly.
        gap_frames = max(1, int(fps * 0.3))
        turn_mask = self._expand_from_core(core_mask, support_mask)
        turn_mask = self._bridge_short_gaps(turn_mask, gap_frames)
        turn_mask = self._filter_short_segments(turn_mask, fps)
        segments, turn_mask = self._extract_segments(
            turn_mask,
            core_mask,
            slow_support_mask,
            body_angle,
        ...
        return segments, turn_mask, angular_speed
    ...
    def _compute_directional_context(
        ...
        n_frames = len(body_angle)
        if n_frames == 0:
            return np.zeros(0, dtype=float), np.zeros(0, dtype=float)

        half_window = max(1, int(round(self.context_window_sec * fps / 2.0)))
        context_rotation_deg = np.zeros(n_frames, dtype=float)
        direction_consistency = np.zeros(n_frames, dtype=float)

        for idx in range(n_frames):
            start = max(0, idx - half_window)
            end = min(n_frames, idx + half_window + 1)
            if end - start <= 1:
                continue
            seg_angle = body_angle[start:end]
            seg_vel = angular_velocity[start:end]
            context_rotation_deg[idx] = abs(float(np.degrees(seg_angle[-1] - seg_angle[0])))
            pos_ratio = float(np.mean(seg_vel > 5.0))
            neg_ratio = float(np.mean(seg_vel < -5.0))
            direction_consistency[idx] = max(pos_ratio, neg_ratio)

        return context_rotation_deg, direction_consistency

    def _compute_body_angle(self, keypoints: np.ndarray) -> np.ndarray:
        shoulder_vec = (
            keypoints[:, self.RIGHT_SHOULDER, :2] - keypoints[:, self.LEFT_SHOULDER, :2]
        )
        hip_vec = keypoints[:, self.RIGHT_HIP, :2] - keypoints[:, self.LEFT_HIP, :2]

        shoulder_angle = np.arctan2(shoulder_vec[:, 1], shoulder_vec[:, 0])
        hip_angle = np.arctan2(hip_vec[:, 1], hip_vec[:, 0])

        # Torso left/right labels can occasionally swap, which creates artificial
        # 180-degree flips if we treat the segment direction as oriented. Use an
        # axis-angle representation (pi-periodic) so the turn detector tracks body
        # orientation rather than left-vs-right ordering artifacts.
        shoulder_axis = np.exp(2j * shoulder_angle)
        hip_axis = np.exp(2j * hip_angle)
        body_axis = shoulder_axis + hip_axis
        body_angle = 0.5 * np.angle(body_axis)
        return np.unwrap(body_angle)

    def _bridge_short_gaps(self, mask: np.ndarray, max_gap_frames: int) -> np.ndarray:
        result = mask.copy()
        changes = np.diff(np.concatenate([[0], mask.astype(int), [0]]))
        starts = np.where(changes == 1)[0]
        ends = np.where(changes == -1)[0]
        for prev_end, next_start in zip(ends[:-1], starts[1:]):
            if 0 < next_start - prev_end <= max_gap_frames:
                result[prev_end:next_start] = True
        return result

    def _expand_from_core(self, core_mask: np.ndarray, support_mask: np.ndarray) -> np.ndarray:
        result = np.zeros_like(core_mask, dtype=bool)
        core_idx = np.flatnonzero(core_mask)
        for idx in core_idx:
            start = idx
            end = idx + 1
            while start > 0 and support_mask[start - 1]:
                start -= 1
            while end < len(support_mask) and support_mask[end]:
                end += 1
            result[start:end] = True
        return result

    def _filter_short_segments(self, mask: np.ndarray, fps: float) -> np.ndarray:
        min_frames = int(self.min_duration * fps)
        result = mask.copy()
        changes = np.diff(np.concatenate([[0], mask.astype(int), [0]]))
        starts = np.where(changes == 1)[0]
    ...
    def _extract_segments(
        ...
        slow_support_mask: np.ndarray,
        body_angle: np.ndarray,
        angular_velocity: np.ndarray,
        angular_speed: np.ndarray,
        fps: float,
    ) -> tuple[list[TurnSegment], np.ndarray]:
        segments = []
        refined_mask = np.zeros_like(mask, dtype=bool)
        changes = np.diff(np.concatenate([[0], mask.astype(int), [0]]))
        starts = np.where(changes == 1)[0]
        ends = np.where(changes == -1)[0]
        pad_frames = max(1, int(round(self.padding_duration * fps)))

        for start, end in zip(starts, ends):
            core_indices = np.flatnonzero(core_mask[start:end])
            if core_indices.size == 0:
                continue
            refined_start = max(start, start + int(core_indices[0]) - pad_frames)
            refined_end = min(end, start + int(core_indices[-1]) + 1 + pad_frames)
            if refined_end - refined_start <= 1:
                continue

            seg_velocity = angular_velocity[refined_start:refined_end]
            seg_speed = angular_speed[refined_start:refined_end]
            if len(seg_speed) == 0:
                continue
            seg_angles = body_angle[refined_start:refined_end]
            if len(seg_angles) == 0:
                continue
            total_rotation_deg = float(np.degrees(np.ptp(seg_angles)))
            net_rotation_deg = abs(float(np.degrees(seg_angles[-1] - seg_angles[0])))
            rotation_extent_deg = max(total_rotation_deg, net_rotation_deg)
            min_rotation_required = self.min_rotation_span_deg
            if np.any(slow_support_mask[refined_start:refined_end]):
                min_rotation_required = min(min_rotation_required, self.slow_rotation_span_deg)
            if rotation_extent_deg < min_rotation_required:
                continue
            signed_mean = float(np.mean(seg_velocity))
            direction = "left" if signed_mean > 0 else "right"
            confidence = float(
                np.clip(np.mean(seg_speed) / max(self.angular_speed_threshold * 2.0, 1.0), 0.0, 1.0)
            )
            segments.append(
                TurnSegment(
                    start_frame=int(refined_start),
                    end_frame=int(refined_end),
                    start_time=float(refined_start / fps),
                    end_time=float(refined_end / fps),
                    confidence=confidence,
                    mean_angular_speed_deg_s=float(np.mean(np.abs(seg_velocity))),
                    peak_angular_speed_deg_s=float(np.max(seg_speed)),
                    total_rotation_deg=rotation_extent_deg,
                    direction=direction,
                )
            )
            refined_mask[refined_start:refined_end] = True

        return segments, refined_mask


class TurnAnalyzer:
    """Analyze turn segments and estimate freezing risk during turning."""
    ...
    def analyze(
        ...
        angular_speed: np.ndarray,
        walking_mask: Optional[np.ndarray],
        fps: float,
    ) -> list[TurnAnalysisResult]:
        if len(keypoints) == 0 or not turn_segments:
            return []
        ...
        ankle_left_rel = ankle_left - hip_center
        ankle_right_rel = ankle_right - hip_center
        ankle_center_rel = (ankle_left_rel + ankle_right_rel) / 2.0
        left_motion = np.sqrt(np.sum(np.diff(ankle_left_rel, axis=0) ** 2, axis=1))
        left_motion = np.concatenate([[0.0], left_motion]) * fps
        right_motion = np.sqrt(np.sum(np.diff(ankle_right_rel, axis=0) ** 2, axis=1))
        right_motion = np.concatenate([[0.0], right_motion]) * fps
        ankle_motion = np.sqrt(np.sum(np.diff(ankle_center_rel, axis=0) ** 2, axis=1))
        ankle_motion = np.concatenate([[0.0], ankle_motion]) * fps

        positive_motion = ankle_motion[ankle_motion > 0]
        default_low_motion_threshold = (
            float(np.percentile(positive_motion, 35)) if positive_motion.size else 0.0
        )
        walking_motion_baseline = None
        if walking_mask is not None and len(walking_mask) == len(ankle_motion):
            walking_motion = ankle_motion[walking_mask]
            walking_motion = walking_motion[walking_motion > 0]
            if walking_motion.size:
                walking_motion_baseline = float(np.median(walking_motion))

        results = []
        neighborhood_frames = max(1, int(round(0.75 * fps)))
        for idx, segment in enumerate(turn_segments, 1):
            start, end = segment.start_frame, segment.end_frame
            seg_motion = ankle_motion[start:end]
            seg_angular = angular_speed[start:end]
            seg_ankle_center = ankle_center_rel[start:end]
            seg_left_motion = left_motion[start:end]
            seg_right_motion = right_motion[start:end]
            if len(seg_motion) == 0:
                continue

            low_motion_threshold = default_low_motion_threshold
            if walking_motion_baseline is not None and walking_motion_baseline > 0:
                low_motion_threshold = max(
                    default_low_motion_threshold,
                    walking_motion_baseline * 0.6,
                )
            low_motion_ratio = float(np.mean(seg_motion <= low_motion_threshold))
            seg_motion_mean = float(np.mean(seg_motion)) if len(seg_motion) else 0.0
            seg_motion_low_quantile = (
                float(np.percentile(seg_motion, 30)) if len(seg_motion) else 0.0
            )
            if walking_motion_baseline is not None and walking_motion_baseline > 0:
                motion_suppression_score = float(
                    np.clip(
                        1.0 - (min(seg_motion_mean, seg_motion_low_quantile) / walking_motion_baseline),
                        0.0,
                        1.0,
                    )
                )
            else:
                positive_seg_motion = seg_motion[seg_motion > 0]
                fallback_baseline = (
                    float(np.percentile(positive_motion, 65)) if positive_motion.size else 0.0
                )
                motion_suppression_score = (
                    float(
                        np.clip(
                            1.0 - (min(seg_motion_mean, seg_motion_low_quantile) / fallback_baseline),
                            0.0,
                            1.0,
                        )
                    )
                    if fallback_baseline > 0
                    else 0.0
                )
            path_length = float(np.sum(seg_motion) / max(fps, 1e-6))
            if len(seg_ankle_center) >= 2:
                net_displacement = float(
                    np.linalg.norm(seg_ankle_center[-1] - seg_ankle_center[0])
                )
            else:
                net_displacement = 0.0
            progression = max(path_length, net_displacement)
            expected_progress = max(segment.total_rotation_deg / 180.0, 0.25)
            rotation_motion_mismatch_score = float(
                np.clip(1.0 - (progression / expected_progress), 0.0, 1.0)
            )
            active_threshold = max(low_motion_threshold, default_low_motion_threshold, 1e-6)
            left_active = seg_left_motion > active_threshold
            right_active = seg_right_motion > active_threshold
            alternation_signal = left_active.astype(int) - right_active.astype(int)
            alternation_changes = int(
                np.sum(np.abs(np.diff(alternation_signal[np.abs(alternation_signal) > 0])) > 0)
            )
            expected_alternations = max(1.0, segment.duration * 1.5)
            # Literature-informed proxies for turn-triggered FOG:
            # - turning is a common FOG trigger
            # - ineffective stepping includes very short shuffling steps
            # - alternating trembling / multiple APAs reflect failed coupling to stepping
            step_irregularity_score = float(
                np.clip(1.0 - (alternation_changes / expected_alternations), 0.0, 1.0)
            )
            small_step_threshold = max(active_threshold * 1.35, 1e-6)
            active_frames = (seg_left_motion > 0) | (seg_right_motion > 0)
            shuffle_frames = active_frames & (seg_left_motion < small_step_threshold) & (
                seg_right_motion < small_step_threshold
            )
            shuffle_step_score = float(np.mean(shuffle_frames)) if len(shuffle_frames) else 0.0
            turn_rotation_threshold = max(
                float(np.mean(seg_angular)) * 0.65 if len(seg_angular) else 0.0,
                self._safe_turn_speed_threshold(default_low_motion_threshold),
            )
            stall_frames = (seg_motion <= low_motion_threshold * 1.1) & (
                seg_angular >= turn_rotation_threshold
            )
            stall_during_rotation_score = float(np.mean(stall_frames)) if len(stall_frames) else 0.0
            legacy_freeze_risk_score = float(
                np.clip(
                    0.65 * low_motion_ratio + 0.35 * min(1.0, segment.duration / 3.0),
                    0.0,
                    1.0,
                )
            )
            suppression_freeze_risk_score = float(
                np.clip(
                    # Weighting emphasizes ineffective stepping first, then
                    # turning-specific failure patterns. These are literature-informed
                    # heuristics rather than validated clinical coefficients.
                    0.22 * low_motion_ratio
                    + 0.18 * motion_suppression_score
                    + 0.18 * rotation_motion_mismatch_score
                    + 0.14 * step_irregularity_score
                    + 0.10 * shuffle_step_score
                    + 0.12 * stall_during_rotation_score
                    + 0.06 * min(1.0, segment.duration / 2.5),
                    0.0,
                    1.0,
                )
            )
            freeze_risk_score = max(
                legacy_freeze_risk_score,
                suppression_freeze_risk_score,
                float(
                    np.clip(
                        0.5 * stall_during_rotation_score
                        + 0.3 * motion_suppression_score
                        + 0.2 * step_irregularity_score,
                        0.0,
                        1.0,
                    )
                ),
            )
            walking_overlap_ratio = 0.0
            nearby_walking_ratio = 0.0
            if walking_mask is not None and end > start:
                walking_overlap_ratio = float(np.mean(walking_mask[start:end]))
                nearby_start = max(0, start - neighborhood_frames)
                nearby_end = min(len(walking_mask), end + neighborhood_frames)
                nearby_walking_ratio = float(np.mean(walking_mask[nearby_start:nearby_end]))
            turn_context = (
                "turning_while_walking"
                if max(walking_overlap_ratio, nearby_walking_ratio) >= 0.25
                else "turning_in_place"
            )

            if freeze_risk_score >= 0.55:
                classification = "possible_fog_turn"
            elif freeze_risk_score >= 0.3:
                classification = "hesitant_turn"
            else:
                classification = "normal_turn"

            results.append(
                TurnAnalysisResult(
                    segment_id=f"turn_{idx}",
                    start_time=segment.start_time,
                    end_time=segment.end_time,
                    duration=segment.duration,
                    direction=segment.direction,
                    turn_context=turn_context,
                    total_rotation_deg=segment.total_rotation_deg,
                    mean_angular_speed_deg_s=float(np.mean(seg_angular)) if len(seg_angular) else 0.0,
                    peak_angular_speed_deg_s=float(np.max(seg_angular)) if len(seg_angular) else 0.0,
                    low_motion_ratio=low_motion_ratio,
                    motion_suppression_score=motion_suppression_score,
                    rotation_motion_mismatch_score=rotation_motion_mismatch_score,
                    step_irregularity_score=step_irregularity_score,
                    shuffle_step_score=shuffle_step_score,
                    stall_during_rotation_score=stall_during_rotation_score,
                    freeze_risk_score=freeze_risk_score,
                    classification=classification,
                    confidence=segment.confidence,
                    debug={
                        "low_motion_threshold": low_motion_threshold,
                        "default_low_motion_threshold": default_low_motion_threshold,
                        "walking_motion_baseline": walking_motion_baseline,
                        "seg_motion_mean": seg_motion_mean,
                        "seg_motion_p30": seg_motion_low_quantile,
                        "path_length": path_length,
                        "net_displacement": net_displacement,
                        "expected_progress": expected_progress,
                        "active_threshold": active_threshold,
                        "alternation_changes": alternation_changes,
                        "expected_alternations": expected_alternations,
                        "small_step_threshold": small_step_threshold,
                        "turn_rotation_threshold": turn_rotation_threshold,
                        "stall_frame_ratio": stall_during_rotation_score,
                        "legacy_freeze_risk_score": legacy_freeze_risk_score,
                        "suppression_freeze_risk_score": suppression_freeze_risk_score,
                    },
                )
            )
        return results

    def _safe_turn_speed_threshold(self, motion_threshold: float) -> float:
        return max(20.0, motion_threshold * 2.0)


class GaitParameterEstimator:
    """Estimate literature-backed gait biomarkers from monocular pose sequences."""
    ...
    def estimate(self, keypoints: np.ndarray, fps: float) -> Dict:
        ...
        # Estimate cadence directly from periodicity, then derive speed from the
        # standard relation speed = stride_length * cadence / 120.
        cadence = self._estimate_cadence(ankle_left, ankle_right, fps)
        speed = (stride_length * cadence) / 120.0 if stride_length > 0 else 0.0
        ...
        # ============ LITERATURE-THRESHOLD ABNORMALITY FLAGS ============
    ...
    def _calculate_pd_indicators(
        ...
        """Flag gait abnormalities using literature-reported cutoffs."""
        del speed, stride_length, asymmetry, step_width
        ...
        for key, ref in self.LITERATURE_THRESHOLDS.items():
            value = float(raw_values[key])
            is_abnormal = value > float(ref["threshold"])
            indicators[key] = {
                "value": value,
        ...
        abnormal_count = sum(1 for item in indicators.values() if item["status"] == "abnormal")
        risk_level = (
            "High" if abnormal_count >= 2 else "Moderate" if abnormal_count == 1 else "Low"
        )
        indicators["overall"] = {
            "abnormal_count": abnormal_count,
            "primary_abnormal": abnormal_count,
            "secondary_abnormal": 0,
            "risk_score": round(abnormal_count / 3.0, 3),
            "risk_level": risk_level,
            "basis": "literature_cutoffs",
        }
        return indicators
    ...
    def classify(self, params: Dict, hgb_runtime: Optional[Dict] = None) -> Dict:
        """Classify gait using literature cutoffs and optional model inference."""
        pd_indicators = params.get("pd_indicators", {})
        overall = pd_indicators.get("overall", {})
        abnormal_count = int(overall.get("abnormal_count", 0))
        risk = str(overall.get("risk_level", "Low"))
        classification = (
            "Multiple literature cutoffs abnormal"
            if abnormal_count >= 2
            else "Single literature cutoff abnormal"
            if abnormal_count == 1
            else "No literature cutoff abnormal"
        )
        percentile = 50
        model_summary = None

        if hgb_runtime and hgb_runtime.get("available") and hgb_runtime.get("prob_pd") is not None:
            prob_pd = float(hgb_runtime["prob_pd"])
            model_summary = "Model-supported PD gait risk" if prob_pd >= float(
                hgb_runtime.get("threshold", 0.5)
            ) else "Model-supported low PD gait risk"
            percentile = int(np.clip((1.0 - prob_pd) * 100.0, 0, 100))
        ...
        return {
            ...
            "pd_indicators_summary": {
                "stride_time_cv": pd_indicators.get("stride_time_cv", {}).get("status", "unknown"),
                "arm_swing_asymmetry": pd_indicators.get("arm_swing_asymmetry", {}).get(
                    "status", "unknown"
                ),
                "step_time_asymmetry": pd_indicators.get("step_time_asymmetry", {}).get(
                    "status", "unknown"
                ),
                "combined": "abnormal" if abnormal_count > 0 else "normal",
            },
            "comparison": {},
            "calibration_note": "Literature cutoff summary; optional runtime model shown separately.",
        }
...
class SmartGaitAnalyzer:
    ...
    def __init__(
        ...
        self.pose_backend = get_pose_backend()
    ...
    def _collect_pose_observations(
        self,
        video_path: str,
        identify_user: bool,
        pose_backend: str,
        pose_sample_rate: int,
        progress_callback=None,
    ) -> Tuple[np.ndarray, np.ndarray, list, int, float, int, int]:
        ...
        with PoseExtractor(backend=pose_backend, use_mediapipe=True) as extractor:
            pose_sequence = extractor.extract_from_video(video_path, sample_rate=pose_sample_rate)
        ...
        while cap.isOpened():
            ...
            sampled_idx = frame_idx // pose_sample_rate
            frame_pose = (
                pose_sequence.frames[sampled_idx].keypoints
                if sampled_idx < len(pose_sequence.frames)
                else None
            )
            if frame_pose is not None and np.asarray(frame_pose).shape == (33, 3):
                frame_pose_np = np.asarray(frame_pose, dtype=float)
                keypoints_list.append(frame_pose_np)
                pose_quality_list.append(
                    self._compute_frame_pose_quality(frame_pose_np, width, height)
                )
            else:
                keypoints_list.append(np.zeros((33, 3), dtype=float))
        ...
        return (
            ...
            np.array(pose_quality_list, dtype=float),
            face_detections,
            total_frames,
            fps,
            width,
    ...
    def _compute_frame_pose_quality(self, keypoints: np.ndarray, width: int, height: int) -> float:
        """Estimate full-body pose quality from keypoints and confidence scores."""
        if keypoints is None or np.asarray(keypoints).shape != (33, 3):
            return 0.0
        ...
        vis = np.clip(np.asarray(keypoints[:, 2], dtype=float), 0.0, 1.0)
        coords = np.asarray(keypoints[required, :2], dtype=float)
    ...
    def _resample_mask(self, mask: np.ndarray, src_fps: float, target_fps: float) -> np.ndarray:
        if len(mask) < 2:
            return mask.astype(bool)
        resampled = self._resample_timeseries(
            mask.astype(float).reshape(-1, 1), src_fps=src_fps, target_fps=target_fps
        ).reshape(-1)
        return resampled >= 0.5

    def _extract_binary_segments(self, mask: np.ndarray, fps: float, label: str) -> list[Dict]:
        segments = []
        changes = np.diff(np.concatenate([[0], mask.astype(int), [0]]))
        starts = np.where(changes == 1)[0]
        ends = np.where(changes == -1)[0]
    ...
    def _build_activity_timeline(
        self, walking_mask: np.ndarray, turn_mask: np.ndarray, fps: float
    ) -> Dict:
        n_frames = max(len(walking_mask), len(turn_mask))
        if n_frames == 0:
            return {
                "fps": fps,
                "segments": [],
        ...
        if len(walking_mask) != n_frames:
            padded = np.zeros(n_frames, dtype=bool)
            padded[: min(len(walking_mask), n_frames)] = walking_mask[: min(len(walking_mask), n_frames)]
            walking_mask = padded
        if len(turn_mask) != n_frames:
            padded = np.zeros(n_frames, dtype=bool)
            padded[: min(len(turn_mask), n_frames)] = turn_mask[: min(len(turn_mask), n_frames)]
            turn_mask = padded

        standing_mask = ~(walking_mask | turn_mask)
        segments = (
            self._extract_binary_segments(walking_mask, fps, "walking")
            + self._extract_binary_segments(turn_mask, fps, "turning")
            + self._extract_binary_segments(standing_mask, fps, "standing")
        )
        ...
        return {
            ...
            "segments": segments,
            "summary": {
                "walking_time": float(np.sum(walking_mask) / fps),
                "turning_time": float(np.sum(turn_mask) / fps),
                "standing_time": float(np.sum(standing_mask) / fps),
                "walking_ratio": float(np.mean(walking_mask)),
                "turning_ratio": float(np.mean(turn_mask)),
                "standing_ratio": float(np.mean(standing_mask)),
            },
        }

    def _summarize_activity_timeline(self, segments: list[Dict], fps: float) -> Dict:
        total_duration = 0.0
        totals = {"walking": 0.0, "turning": 0.0, "standing": 0.0}
        for seg in segments:
            label = str(seg.get("label") or seg.get("activity_type") or "standing")
            duration = float(seg.get("duration", max(0.0, float(seg.get("end_time", 0.0)) - float(seg.get("start_time", 0.0)))))
            total_duration += duration
            if label in totals:
                totals[label] += duration
        denom = max(total_duration, 1e-6)
        return {
            "walking_time": float(totals["walking"]),
            "turning_time": float(totals["turning"]),
            "standing_time": float(totals["standing"]),
            "walking_ratio": float(totals["walking"] / denom),
            "turning_ratio": float(totals["turning"] / denom),
            "standing_ratio": float(totals["standing"] / denom),
        }

    def _promote_directional_gap_turns(
        self,
        activity_timeline: Dict,
        keypoints: np.ndarray,
        fps: float,
    ) -> Dict:
        segments = [dict(seg) for seg in activity_timeline.get("segments", [])]
        if len(segments) < 3 or len(keypoints) == 0:
            return activity_timeline

        body_angle = self.turn_detector._compute_body_angle(keypoints)
        angular_velocity = np.gradient(body_angle) * fps * (180.0 / np.pi)
        ankle_center = (
            keypoints[:, WalkingDetector.ANKLE_LEFT, :2] + keypoints[:, WalkingDetector.ANKLE_RIGHT, :2]
        ) / 2.0
        ankle_motion = np.sqrt(np.sum(np.diff(ankle_center, axis=0) ** 2, axis=1))
        ankle_motion = np.concatenate([[0.0], ankle_motion]) * fps
        ...
        for idx in range(1, len(segments) - 1):
            ...
            if label != "standing":
                continue
            if str(prev_seg.get("label") or prev_seg.get("activity_type")) != "walking":
                continue
            if str(next_seg.get("label") or next_seg.get("activity_type")) != "walking":
                continue

            start = int(seg.get("start_frame", 0))
            end = int(seg.get("end_frame", start))
            if end - start < max(1, int(round(1.0 * fps))):
                continue

            seg_angles = body_angle[start:end]
            seg_velocity = angular_velocity[start:end]
            seg_motion = ankle_motion[start:end]
            if len(seg_angles) <= 1 or len(seg_motion) == 0:
                continue

            net_rotation_deg = abs(float(np.degrees(seg_angles[-1] - seg_angles[0])))
            if net_rotation_deg >= self.turn_detector.slow_rotation_span_deg:
                seg["label"] = "turning"
                changed = True
        ...
        return {
            "fps": fps,
            "segments": segments,
            "summary": self._summarize_activity_timeline(segments, fps),
        }

    def _build_turn_segments_from_timeline(
        self,
        activity_timeline: Dict,
        body_angle: np.ndarray,
        angular_velocity: np.ndarray,
        ...
        turn_segments: list[TurnSegment] = []
        for seg in activity_timeline.get("segments", []):
            if str(seg.get("label") or seg.get("activity_type") or "") != "turning":
                continue
            start = int(seg.get("start_frame", 0))
            end = int(seg.get("end_frame", start))
            if end - start <= 1:
            ...
            seg_angles = body_angle[start:end]
            seg_velocity = angular_velocity[start:end]
            seg_speed = angular_speed[start:end]
            if len(seg_angles) <= 1 or len(seg_speed) == 0:
                continue
            total_rotation_deg = max(
                abs(float(np.degrees(seg_angles[-1] - seg_angles[0]))),
                float(np.degrees(np.ptp(seg_angles))),
            )
            signed_mean = float(np.mean(seg_velocity))
            direction = "left" if signed_mean > 0 else "right"
            confidence = float(
                np.clip(np.mean(np.abs(seg_velocity)) / max(self.turn_detector.angular_speed_threshold * 2.0, 1.0), 0.0, 1.0)
            )
    ...
    def _summarize_turn_analysis(
        self, turn_segments: List[TurnSegment], turn_results: List[TurnAnalysisResult]
    ) -> Dict:
        if not turn_segments:
            return {
                "total_segments": 0,
                "total_turn_time": 0.0,
                "possible_fog_turns": 0,
                "hesitant_turns": 0,
                "normal_turns": 0,
                "turning_while_walking": 0,
                "turning_in_place": 0,
            }

        return {
            "total_segments": len(turn_segments),
            "total_turn_time": float(sum(seg.duration for seg in turn_segments)),
            "possible_fog_turns": sum(r.classification == "possible_fog_turn" for r in turn_results),
            "hesitant_turns": sum(r.classification == "hesitant_turn" for r in turn_results),
            "normal_turns": sum(r.classification == "normal_turn" for r in turn_results),
            "turning_while_walking": sum(
                r.turn_context == "turning_while_walking" for r in turn_results
            ),
            "turning_in_place": sum(r.turn_context == "turning_in_place" for r in turn_results),
        }

    def _turn_methodology(self) -> Dict:
        return {
            "purpose": "screening",
            "scope": "turn-focused freezing-of-gait risk heuristics from monocular pose",
            "direct_literature_support": [
                "turning is a common trigger of freezing of gait",
                "freezing of gait involves ineffective stepping",
            ...
            "heuristic_components": [
                "score weights",
                "possible_fog_turn threshold",
                "rotation_motion_mismatch_score formula",
                "left-right alternation failure proxy",
                "shuffle-like stepping proxy from pose velocity",
            ],
            "features": [
                "low_motion_ratio",
                "motion_suppression_score",
            ...
            "references": [
                {
                    ...
                    "pubmed": "https://pubmed.ncbi.nlm.nih.gov/21777828/",
                },
                {
                    ...
                    "pubmed": "https://pubmed.ncbi.nlm.nih.gov/36354550/",
                },
                {
                    ...
                    "pubmed": "https://pubmed.ncbi.nlm.nih.gov/22135799/",
                },
                {
                    ...
                    "pubmed": "https://pubmed.ncbi.nlm.nih.gov/22342993/",
                },
            ],
            "limitations": [
                "not a validated clinical diagnostic model",
                "depends on monocular pose quality and camera viewpoint",
                "turn scores are literature-informed heuristics rather than validated coefficients",
                "medical interpretation requires validation on labeled turning-FOG datasets",
            ],
        }
    ...
    def _build_review_skeleton_track_payload(
        self,
        raw_keypoints: np.ndarray,
        raw_pose_quality: np.ndarray | None,
        fps: float,
    ) -> Dict:
        """Build a UI overlay track aligned to the gait pipeline timebase.

        Unlike `_apply_skeleton_pipeline`, this keeps image-space coordinates so the
        frontend can render over the original video while sharing the gait timeline fps.
        """
        if raw_pose_quality is None:
            raw_pose_quality = np.zeros(len(raw_keypoints), dtype=float)

        keypoints = np.asarray(raw_keypoints, dtype=float)
        pose_quality = np.asarray(raw_pose_quality, dtype=float).reshape(-1)
        if keypoints.ndim != 3 or len(keypoints) == 0:
            return {}

        review_keypoints = self._resample_timeseries(
            keypoints, src_fps=fps, target_fps=self.pipeline_target_fps
        )

        review_quality = self._resample_timeseries(
            pose_quality.reshape(-1, 1), src_fps=fps, target_fps=self.pipeline_target_fps
        ).reshape(-1)
        review_quality = np.clip(review_quality, 0.0, 1.0)

        n_frames = int(review_keypoints.shape[0])
        frames = np.round(review_keypoints, 4).tolist()
        return {
            "start_time": 0.0,
            "end_time": float(max(0.0, (n_frames - 1) / self.pipeline_target_fps)),
            "start_frame": 0,
            "end_frame": max(0, n_frames - 1),
            "fps": float(self.pipeline_target_fps),
            "frame_stride": 1,
            "n_frames": n_frames,
            "n_landmarks": int(review_keypoints.shape[1]),
            "frames": frames,
            "keypoints": frames,
            "pose_quality": np.round(review_quality, 4).tolist(),
        }

    def _shift_analysis_output(
        self,
        payload: Dict,
        ...
        if time_offset_sec == 0 and frame_offset == 0 and total_frames is None and video_duration is None:
            return payload
        ...
        def shift_value(obj):
            if isinstance(obj, dict):
                ...
                for key, value in obj.items():
                    if key in {"start_time", "end_time", "boundary_time"} and value is not None:
                        shifted[key] = float(value) + time_offset_sec
                    elif key in {
                        "start_frame",
                        "end_frame",
                        "current_start_frame",
                        ...
                        shifted[key] = int(value) + frame_offset
                    else:
                        shifted[key] = shift_value(value)
                return shifted
            if isinstance(obj, list):
                return [shift_value(item) for item in obj]
            return obj

        shifted = shift_value(payload)
        video_info = shifted.get("video_info", {})
        if total_frames is not None:
            video_info["total_frames"] = int(total_frames)
        if video_duration is not None:
            video_info["duration"] = float(video_duration)
        if video_info:
            shifted["video_info"] = video_info
        return shifted

    def _analyze_pose_series(
        self,
        raw_keypoints: np.ndarray,
        raw_pose_quality: np.ndarray,
        fps: float,
        total_frames: int,
        width: int,
        height: int,
        video_path: str,
        active_pose_backend: str,
        identify_user: bool,
        ...
        keypoints, pose_quality, analysis_fps, pipeline_meta = self._apply_skeleton_pipeline(
            raw_keypoints, raw_pose_quality, fps
        )
        body_angle = self.turn_detector._compute_body_angle(keypoints)
        angular_velocity = np.gradient(body_angle) * analysis_fps * (180.0 / np.pi)

        turn_segments, raw_turn_mask, raw_turn_speed = self.turn_detector.detect(
            raw_keypoints, raw_pose_quality, fps
        )
        turn_mask = self._resample_mask(raw_turn_mask, fps, analysis_fps)
        turn_speed = uniform_filter1d(np.abs(angular_velocity), max(3, int(analysis_fps * 0.25)))

        _, detected_walking_mask, walking_confidence = self.walking_detector.detect(
            keypoints, analysis_fps
        )
        walking_without_turns = detected_walking_mask & ~turn_mask
        walking_without_turns = self.walking_detector._filter_short_segments(
            walking_without_turns, analysis_fps
        )
        segments = self.walking_detector._extract_segments(
            walking_without_turns, walking_confidence, analysis_fps
        )
        activity_timeline = self._build_activity_timeline(
            walking_without_turns, turn_mask, analysis_fps
        )
        activity_timeline = self._promote_directional_gap_turns(
            activity_timeline, keypoints, analysis_fps
        )
        refined_turn_segments = self._build_turn_segments_from_timeline(
            activity_timeline,
        ...
        turn_results = self.turn_analyzer.analyze(
            keypoints, refined_turn_segments, turn_speed, detected_walking_mask, analysis_fps
        )
        turn_detection_summary = self._summarize_turn_analysis(refined_turn_segments, turn_results)
        ...
        for i, segment in enumerate(segments):
                ...
                segment_quality_report.append(
                    {
                        ...
                        "duration": segment.duration,
                        "confidence": segment.confidence,
                        "pose_quality_mean": float(np.mean(seg_quality)) if len(seg_quality) else 0.0,
            ...
            classification = estimator.classify(params, hgb_runtime=hgb_runtime)

            user_profile = self.face_recognizer.get_user(identified_user) if identified_user else None
            pd_indicators = params.get("pd_indicators", {})
        ...
        return _convert_numpy(
            {
                "pose_backend": active_pose_backend,
                "pose_backend_requested": self.pose_backend,
                "pose_backend_used": active_pose_backend,
                "video_info": {
                    "path": video_path,
                    "fps": fps,
                ...
                "user": {
                    ...
                    "user_id": identified_user,
                    "user_name": self.face_recognizer.get_user(identified_user).name if identified_user else None,
                ...
                "walking_detection": {
                    ...
                    "excluded_low_quality_segments": max(0, len(segments) - total_quality_valid_segments),
                    "total_walking_time": sum(s.duration for s in segments),
                    "walking_ratio": activity_timeline["summary"]["walking_ratio"],
                ...
                "turn_detection": {
                    "segments": [seg.to_dict() for seg in refined_turn_segments],
                    "summary": turn_detection_summary,
                },
                "turn_analysis": [result.to_dict() for result in turn_results],
                "turn_methodology": self._turn_methodology(),
                "activity_timeline": activity_timeline,
                "preprocessing": pipeline_meta,
                "backend_selection": {
                    "requested": self.pose_backend,
                    "used": active_pose_backend,
    ...
    def analyze_precomputed_pose(
        ...
        if raw_pose_quality is None:
            raw_pose_quality = np.zeros(len(raw_keypoints), dtype=float)
        gait_review_track = self._build_review_skeleton_track_payload(
            raw_keypoints=raw_keypoints,
            raw_pose_quality=raw_pose_quality,
            fps=fps,
        )
        result = self._analyze_pose_series(
            raw_keypoints=raw_keypoints,
            raw_pose_quality=raw_pose_quality,
            fps=fps,
            total_frames=total_frames,
            ...
            active_pose_backend=pose_backend,
            identify_user=False,
            face_detections=[],
            user_height_cm=user_height_cm,
            fallback_used=False,
        )
        if gait_review_track:
            result["gait_review_track"] = gait_review_track
        return self._shift_analysis_output(
            result,
    ...
    def analyze_video(
        ...
        pose_sample_rate = max(1, int(os.getenv("POSE_SAMPLE_RATE", "1")))
        active_pose_backend = self.pose_backend
        (
            raw_keypoints,
            raw_pose_quality,
            face_detections,
            ...
            width,
            height,
        ) = self._collect_pose_observations(
            video_path,
            identify_user=identify_user,
            pose_backend=active_pose_backend,
            pose_sample_rate=pose_sample_rate,
        ...
        fallback_used = False
        if (
            active_pose_backend == "mediapipe"
            and (
                pipeline_meta.get("detection_rate", 0.0) < 0.05
                or pipeline_meta.get("mean_pose_quality", 0.0) < 0.10
            )
        ):
            (
                raw_keypoints,
            ...
            active_pose_backend = "rtmw"
            fallback_used = True
        return self._analyze_pose_series(
            raw_keypoints=raw_keypoints,
            raw_pose_quality=raw_pose_quality,
            fps=fps,
            total_frames=total_frames,
            ...
            height=height,
            video_path=video_path,
            active_pose_backend=active_pose_backend,
            identify_user=identify_user,
...
def get_analyzer(data_dir: str = None, min_segment_duration: float = 1.0) -> SmartGaitAnalyzer:
    ...
    if _analyzer is None:
        if data_dir is None:
            data_dir = Path(__file__).resolve().parents[2] / "runtime" / "data"
