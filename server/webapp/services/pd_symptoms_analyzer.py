...
from ..activity_schema import build_activity_schema
...
class PersonTrack:
    ...
    start_frame: int
    end_frame: int
    keypoints: np.ndarray  # Shape: (frames, 33, 3) in shared pose schema
...
class MultiPersonTracker:
    """Track people across video frames using the configured pose backend."""

    # Shared 33-keypoint pose schema indices
    NOSE = 0
    LEFT_SHOULDER = 11
    RIGHT_SHOULDER = 12
    LEFT_HIP = 23
    ...
    def __init__(self, max_persons: int = 5, min_track_frames: int = 30, pose_backend: Optional[str] = None):
        """
        Args:
        ...
        self.pose_backend = get_pose_backend(pose_backend)

    def _extract_tracks_with_backend(
        self,
        video_path: str,
        backend: str,
        fps: float = None,
        progress_callback=None,
    ) -> Tuple[List[PersonTrack], Dict]:
        ...
        with PoseExtractor(backend=backend, use_mediapipe=True) as extractor:
            while cap.isOpened():
                ret, frame = cap.read()
                if not ret:
                    break

                detections = self._extract_detections(extractor, frame)

                for landmarks in detections[:self.max_persons]:
                    ...
                    scores = np.clip(landmarks[:, 2], 0.0, 1.0)
                    frame_quality = self._compute_frame_pose_quality(landmarks, scores, width, height)
                ...
                for tid, track in active_tracks.items():
                    if frame_idx - track['last_frame'] > fps * 0.5:
    ...
    def extract_tracks(self, video_path: str, fps: float = None,
        ...
        Returns:
            ...
            Video metadata dict
        """
        tracks, video_info = self._extract_tracks_with_backend(
            video_path, self.pose_backend, fps=fps, progress_callback=progress_callback
        )
        if tracks or self.pose_backend != 'mediapipe':
            return tracks, video_info

        fallback_backend = 'rtmw'
        fallback_tracks, fallback_info = self._extract_tracks_with_backend(
            video_path, fallback_backend, fps=fps, progress_callback=progress_callback
        )
        if fallback_tracks:
            fallback_info['pose_backend_requested'] = self.pose_backend
            fallback_info['pose_backend_fallback_used'] = True
            return fallback_tracks, fallback_info

        video_info['pose_backend_requested'] = self.pose_backend
        video_info['pose_backend_fallback_attempted'] = fallback_backend
        video_info['pose_backend_fallback_used'] = False
        return tracks, video_info

    def _extract_detections(self, extractor: PoseExtractor, frame: np.ndarray) -> List[np.ndarray]:
        """Extract one or more pose detections for the current frame."""
        if extractor.backend == 'rtmw' and extractor.wholebody_model is not None:
            model_frame = frame
            resize_scale = 1.0
            if getattr(extractor, 'max_inference_size', 0) > 0:
                h, w = frame.shape[:2]
                longest_side = max(h, w)
                if longest_side > extractor.max_inference_size:
                    resize_scale = extractor.max_inference_size / float(longest_side)
            ...
            keypoints, scores = extractor.wholebody_model(model_frame)
            ...
            for inst_xy, inst_scores in zip(keypoints[:self.max_persons], scores[:self.max_persons]):
                mapped = extractor._openpose_to_mediapipe33(inst_xy, inst_scores)
                if np.count_nonzero(mapped[:, 2] > 0.1) >= 8:
                    detections.append(mapped)
            return detections

        single = extractor._extract_frame(frame)
        if single is None:
            return []
        return [single]
...
class TremorAnalyzer:
    ...
    # Spectral dominance threshold used to mark a candidate rest-tremor band event.
    PD_POWER_RATIO_THRESHOLD = 0.15
    ...
    def analyze(self, track: PersonTrack, segment: AnalysisSegment,
        ...
        # Tremor candidate flag from spectral dominance in the classical PD rest-tremor band.
        ...
        tremor_candidate = bool(is_pd_frequency and has_significant_power)
        severity = 'mild' if tremor_candidate else 'normal'
        ...
        return SymptomResult(
            ...
            metrics={
                ...
                'rest_tremor_candidate': tremor_candidate,
            },
            severity=severity,
...
class PostureAnalyzer:
    ...
    MIN_VALID_RATIO = 0.6
    FRONTAL_VIEW_MIN_RATIO = 0.12
    LATERAL_VIEW_MAX_RATIO = 0.08
    SUSTAINED_RATIO = 0.6
    ...
    def analyze(self, track: PersonTrack, segment: AnalysisSegment,
        ...
        shoulder_width = np.abs(keypoints[:, LEFT_SHOULDER, 0] - keypoints[:, RIGHT_SHOULDER, 0])
        hip_width = np.abs(keypoints[:, LEFT_HIP, 0] - keypoints[:, RIGHT_HIP, 0])
        trunk_height_series = np.linalg.norm(
            ((keypoints[:, LEFT_SHOULDER, :2] + keypoints[:, RIGHT_SHOULDER, :2]) / 2) -
            ((keypoints[:, LEFT_HIP, :2] + keypoints[:, RIGHT_HIP, :2]) / 2),
            axis=1,
        )
        width_ratio = np.divide(
            shoulder_width + hip_width,
            2.0 * (trunk_height_series + 1e-6),
        )
        valid_mask = np.isfinite(width_ratio) & (trunk_height_series > 1.0)
        valid_ratio = float(np.mean(valid_mask)) if len(valid_mask) else 0.0
        if valid_ratio < self.MIN_VALID_RATIO:
            return self._insufficient_view_result(segment, valid_ratio, 'insufficient_visible_torso')

        median_width_ratio = float(np.median(width_ratio[valid_mask]))
        sagittal_view_ok = median_width_ratio <= self.LATERAL_VIEW_MAX_RATIO
        coronal_view_ok = median_width_ratio >= self.FRONTAL_VIEW_MIN_RATIO
        ...
        sustained_forward_ratio = float(np.mean(trunk_angles >= self.LOWER_CAMPTOCORMIA_THRESHOLD))
        ...
        sustained_lateral_ratio = float(np.mean(np.abs(lateral_angles) >= self.PISA_THRESHOLD))

        # Pisa syndrome requires SUSTAINED deviation (consistency check)
        has_pisa_syndrome = (
            coronal_view_ok
            and avg_lateral_angle >= self.PISA_THRESHOLD
            and lateral_consistency > 0.7
            and sustained_lateral_ratio >= self.SUSTAINED_RATIO
        )
        ...
        sustained_head_drop_ratio = float(np.mean(head_angles >= self.MILD_ANTECOLLIS))
        ...
        if sagittal_view_ok and avg_trunk_angle >= self.UPPER_CAMPTOCORMIA_THRESHOLD and sustained_forward_ratio >= self.SUSTAINED_RATIO:
            updrs_score = max(updrs_score, 4)
            severity_reasons.append(f'upper_camptocormia_{avg_trunk_angle:.1f}deg')
        elif sagittal_view_ok and avg_trunk_angle >= self.LOWER_CAMPTOCORMIA_THRESHOLD and sustained_forward_ratio >= self.SUSTAINED_RATIO:
            updrs_score = max(updrs_score, 3)
            severity_reasons.append(f'lower_camptocormia_{avg_trunk_angle:.1f}deg')
        elif sagittal_view_ok and avg_trunk_angle >= self.MODERATE_STOOPED:
            updrs_score = max(updrs_score, 2)
            severity_reasons.append(f'stooped_{avg_trunk_angle:.1f}deg')
        elif sagittal_view_ok and avg_trunk_angle >= self.MILD_STOOPED:
        ...
        # Head drop scoring
        if sagittal_view_ok and avg_head_angle >= self.SEVERE_ANTECOLLIS and sustained_head_drop_ratio >= self.SUSTAINED_RATIO:
            updrs_score = max(updrs_score, 4)
            severity_reasons.append(f'dropped_head_{avg_head_angle:.1f}deg')
        elif sagittal_view_ok and avg_head_angle >= self.MODERATE_ANTECOLLIS and sustained_head_drop_ratio >= self.SUSTAINED_RATIO:
            updrs_score = max(updrs_score, 3)
            severity_reasons.append(f'moderate_antecollis_{avg_head_angle:.1f}deg')
        elif sagittal_view_ok and avg_head_angle >= self.MILD_ANTECOLLIS:
            updrs_score = max(updrs_score, 2)
        ...
        return SymptomResult(
            ...
            metrics={
                ...
                'sustained_forward_ratio': sustained_forward_ratio,
    ...
    def _insufficient_view_result(self, segment: AnalysisSegment, valid_ratio: float, reason: str) -> SymptomResult:
        return SymptomResult(
            symptom_type='posture',
            person_id=segment.person_id,
            segment_id=f"posture_{segment.start_frame}",
            ...
            duration=segment.duration,
            metrics={
                'view_valid_ratio': float(valid_ratio),
                'view_suitability': 'insufficient',
...
class FOGTransitionAnalyzer:
    ...
    def _determine_severity(self, metrics: Dict) -> tuple:
        ...
        if freeze_index >= self.FI_SEVERE:
            ...
            severity = 'normal'
            severity_reasons.append('no_elevated_freeze_index')
...
class GaitAnalyzer:
    """Analyze gait segments using literature-backed variability and asymmetry markers."""
    ...
    STRIDE_TIME_CV_THRESHOLD = 2.6
    STEP_TIME_ASYMMETRY_THRESHOLD = 0.05
    ARM_SWING_ASYMMETRY_THRESHOLD = 0.10
    ...
    def _extract_features(self, keypoints: np.ndarray, fps: float) -> Dict[str, float]:
        ...
        cadence = 0.0
        stride_time_cv = 0.0
        step_time_asymmetry = 0.0

        left_vertical = uniform_filter1d(left_ankle[:, 1], size=max(3, int(fps * 0.15)))
        right_vertical = uniform_filter1d(right_ankle[:, 1], size=max(3, int(fps * 0.15)))
        min_peak_distance = max(1, int(fps * 0.35))
        left_peaks, _ = signal.find_peaks(-left_vertical, distance=min_peak_distance)
        right_peaks, _ = signal.find_peaks(-right_vertical, distance=min_peak_distance)
        all_steps = np.sort(np.concatenate([left_peaks, right_peaks]))

        if len(all_steps) > 1:
            duration_sec = max(len(keypoints) / fps, 1e-6)
            cadence = float(np.clip((len(all_steps) / duration_sec) * 60.0, 0.0, 180.0))
            ...
            if len(step_intervals) > 1:
                stride_pairs = []
                for i in range(0, len(step_intervals) - 1, 2):
                    stride_pairs.append(step_intervals[i] + step_intervals[i + 1])
                if not stride_pairs:
                    stride_pairs = step_intervals.tolist()
                stride_arr = np.asarray(stride_pairs, dtype=float)
                if len(stride_arr) > 1:
                    stride_time_cv = float(
                        (np.std(stride_arr, ddof=1) / (np.mean(stride_arr) + 1e-6)) * 100.0
                    )

        if len(left_peaks) > 1 and len(right_peaks) > 1:
            left_step = np.diff(left_peaks) / fps
            right_step = np.diff(right_peaks) / fps
            left_mean = float(np.mean(left_step))
            right_mean = float(np.mean(right_step))
            step_time_asymmetry = float(
                abs(left_mean - right_mean) / max((left_mean + right_mean) / 2.0, 1e-6)
            )

        shoulder_center = (keypoints[:, self.LEFT_SHOULDER, :2] + keypoints[:, self.RIGHT_SHOULDER, :2]) / 2
        left_arm = np.linalg.norm(keypoints[:, self.LEFT_WRIST, :2] - shoulder_center, axis=1)
        right_arm = np.linalg.norm(keypoints[:, self.RIGHT_WRIST, :2] - shoulder_center, axis=1)
        left_arm_amp = float(np.percentile(left_arm, 95) - np.percentile(left_arm, 5)) if len(left_arm) else 0.0
        right_arm_amp = float(np.percentile(right_arm, 95) - np.percentile(right_arm, 5)) if len(right_arm) else 0.0
        arm_swing_asymmetry = float(
            abs(left_arm_amp - right_arm_amp) / max(left_arm_amp + right_arm_amp, 1e-6)
        )
    ...
    def _classify_severity(self, metrics: Dict[str, float]) -> str:
        abnormalities = 0
        if metrics.get('stride_time_cv', 0.0) > self.STRIDE_TIME_CV_THRESHOLD:
            abnormalities += 1
        if metrics.get('step_time_asymmetry', 0.0) > self.STEP_TIME_ASYMMETRY_THRESHOLD:
            abnormalities += 1
        if metrics.get('arm_swing_asymmetry', 0.0) > self.ARM_SWING_ASYMMETRY_THRESHOLD:
            abnormalities += 1
...
class PDSymptomsAnalyzer:
    ...
    def analyze_tracks(
        ...
        for track in tracks:
            ...
            person_data['activity_schema'] = build_activity_schema(
                routing_segments=person_data['activity_segments'],
                routing_summary=activity_breakdown,
                gait_timeline=None,
                gait_source_person_id=None,
                owner_person_id=track.person_id,
            )
        ...
        return _convert_numpy({
            ...
            'activity_schema': build_activity_schema(
                routing_segments=[],
                routing_summary=activity_summary,
                gait_timeline=None,
                gait_source_person_id=None,
    ...
    def _build_skeleton_track_payload(self, track: PersonTrack, frame_stride: int = 2) -> Dict:
        """Serialize review-pose frames for the UI overlay.

        The UI historically consumed `frames`; newer code also reads `keypoints`.
        Keep both names aligned so saved results and history reloads are stable.
        """
    ...
    def analyze_video(self, video_path: str,
        ...
        return self.analyze_tracks(
            ...
            skeleton_frame_stride=skeleton_frame_stride,
        )
