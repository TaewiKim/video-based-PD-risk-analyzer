...
def _input_processing_metadata(video_filename: str) -> dict:
    source_extension = video_filename.rsplit(".", 1)[1].lower() if "." in video_filename else "unknown"
    return {
        ...
        "audio_removed": True,
        "max_side_px": 320,
        "notice": "Analysis runs on a resized MP4/H.264 copy with audio removed.",
    }


def _build_stroke_screening(summary: dict | None) -> dict:
...
def _attach_review_layers(results: dict) -> dict:
    ...
    results["review_layers"] = {
        "gait_analysis": {
            ...
            "description": "Shared gait metrics and session reliability used as the upstream biomarker layer.",
        },
        "pd_analysis": {
            "title": "PD Analysis",
            "description": "PD-focused branch derived from the common gait analysis layer.",
        },
        "stroke_analysis": {
            "title": "Stroke Analysis",
            "description": "Stroke-linked gait risk branch derived from the common gait analysis layer.",
        },
    }
    return results


def _run_gait_analysis(filename: str, identify_user: bool) -> tuple[dict, str]:
    filepath = UPLOAD_DIR / filename
    analyzer, _ = ensure_analyzers()
    results = analyzer.analyze_video(str(filepath), identify_user=identify_user)
    results = _attach_review_layers(results)
    results["input_processing"] = _input_processing_metadata(filename)
...
def _run_symptom_analysis(filename: str, symptoms) -> tuple[dict, str]:
    filepath = UPLOAD_DIR / filename
...
def api_analyze(request):
    ...
    analyzer, _ = ensure_analyzers()
    allowed, usage = consume_usage(client_key(request))
    if not allowed:
        return JsonResponse(usage, status=429)

    try:
        results, result_filename = _run_gait_analysis(filename, identify_user)
        remember_result_access(request, result_filename)
...
def api_analyze_symptoms(request):
    ...
    try:
        results, result_filename = _run_symptom_analysis(filename, symptoms)
        remember_result_access(request, result_filename)
        return JsonResponse({**results, "usage": usage})
    except Exception as exc:
        return JsonResponse({"error": str(exc), **usage}, status=500)
...
def api_analyze_async(request):
    ...
    if not allowed:
        return JsonResponse(usage, status=429)
    job = create_job(job_type="gait", video_filename=filename)
    remember_job_access(request, job["job_id"])
    run_job(
        job["job_id"],
        lambda: {
            "result_filename": _run_gait_analysis(filename, identify_user)[1],
            "result_type": "gait",
            "video_filename": filename,
        },
    )
    return JsonResponse({**job, "usage": usage}, status=202)


@api_login_required
@csrf_exempt
@require_POST
def api_analyze_symptoms_async(request):
    payload = parse_json_body(request)
    filename = payload.get("filename")
    symptoms = payload.get("symptoms")
    if not filename:
    ...
    if not allowed:
        return JsonResponse(usage, status=429)
    job = create_job(job_type="symptoms", video_filename=filename)
    remember_job_access(request, job["job_id"])
    run_job(
        job["job_id"],
        lambda: {
            "result_filename": _run_symptom_analysis(filename, symptoms)[1],
            "result_type": "symptoms",
            "video_filename": filename,
        },
    )
    return JsonResponse({**job, "usage": usage}, status=202)
...
def api_job_status(request, job_id: str):
    if not can_access_job(request, job_id):
        return JsonResponse({"error": "Forbidden"}, status=403)
    job = get_job(job_id)
    if job is None:
        return JsonResponse({"error": "Job not found"}, status=404)
    if job.get("result_filename"):
        remember_result_access(request, job["result_filename"])
